...
def profile(request):
    ...
    profile = database.child("users").child(a).child('profile').get().val()
    ...
    return render(request, "profile.html", {"fname": fname, "lname": lname, "uname": uname, "pno": pno, "add": add, "city": city, "state": state, "zip": zip, "e": e, "vno": vno, "url": url})
